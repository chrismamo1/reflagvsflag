...
func VoteHandler(db *sql.DB, resps chan things.IDPair) func(http.ResponseWriter, *http.Request) {
    return func(writer http.ResponseWriter, req *http.Request) {
        ...
        query := `
        BEGIN
            (CASE WHEN EXISTS(SELECT * FROM EXPOSURE WHERE "user" = $1 AND image = $2)
            THEN (UPDATE exposure SET heat = heat + 1 WHERE "user" = $3 AND image = $4)
            ELSE (INSERT INTO exposure ("user", image, heat) VALUES ($5, $6, 1))
            END)
        COMMIT;
        `
