...
func JudgeHandler(db *sql.DB, reqs chan things.IDPair) func(http.ResponseWriter, *http.Request) {
        return func(writer http.ResponseWriter, req *http.Request) {
                ...
                left, right := selectImages(db, ids)
                page := `
...
RETRY:
                        ...
                        if l > r {
                                ...
                                if newPivot == -1 || iLeft == iRight || iRight - iLeft <= 1 {
                                        ...
                                        <-isDoneLeft
                                        <-isDoneRight
                                        /*rightPivot := selectPivot(iCenter + 1, iRight)
                                        if rightPivot != -1 {
                                                var aboveCenter things.ID
                                                err = db.QueryRow("SELECT id FROM images WHERE img_index = ?", iCenter + 1).Scan(&aboveCenter)
                                                if err != nil {
                                                        log.Fatal(err)
                                                }
                                                ...
                                                isDoneRight = make(chan bool)
                                                db.QueryRow("SELECT id FROM images WHERE img_index = ?", iRight).Scan(&right)
                                                go quickSort(aboveCenter, rightPivot, right, isDoneRight)
                                        }
                                        if isDoneLeft != nil {
                                                doneLeft := <-isDoneLeft
                                                if doneLeft != true {
                                                        fmt.Println("isDoneLeft returned something fucky")
                                                }
                                        }
                                        if isDoneRight != nil {
                                                doneRight := <-isDoneRight
                                                if doneRight != true {
                                                        fmt.Println("isDoneRight returned something fucky")
                                                }
                                        }*/
                                        done <- true
                                        return
        ...
        for {
                ...
                cmp := things.GetComparison(db, i, pivot)
                if cmp > 0 { // images[i] < images[pivot]
                        tx.Exec("UPDATE images SET index = ? WHERE id = ?", index, left)
                }
...
func main() {
        ...
        reqs := make(chan things.IDPair)
        resps := make(chan things.IDPair)
        ...
        //if len(store) == 0 {
        //        log.Fatal("Something is obviously wrong with the image store.\n")
        //}
        ...
        r.HandleFunc("/judge", JudgeHandler(db, reqs))
        r.HandleFunc("/vote", VoteHandler(db, resps))
        r.HandleFunc("/shutdown", ShutdownHandler(srv, db))
        r.PathPrefix("/static/").Handler(http.StripPrefix("/static/", http.FileServer(http.Dir("static/"))))

        go flagSort(db, reqs, resps)
